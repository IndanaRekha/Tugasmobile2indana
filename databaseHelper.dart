...
class DbStudentManager {
  ...
  Future openDb() async {
    if (_database == null) {
      _database = await openDatabase(join(await getDatabasesPath(), "ss.db"),
        ...
        await db.execute(
          "CREATE TABLE student(id INTEGER PRIMARY KEY autoincrement, name TEXT, pasword TEXT)",
        );
      });
    }
  }
  ...
  Future<List<Student>> getStudentList() async {
    ...
    return List.generate(maps.length, (i) {
      return Student(
          id: maps[i]['id'], name: maps[i]['name'], course: maps[i]['pasword']);
    });
  }
...
class Student {
  ...
  Map<String, dynamic> toMap() {
    return {'name': name, 'pasword': course};
  }
}
